...
/*
 * Draws the body cone for the ships hull
 * translated x, y,z
 * scaled dx, dy, dz
 * spin is for the texture to tell it where to start in the texture image
 * 
 */

void body_cone(double x, double y, double z, double dx, double dy, double dz, 
							 double deg_cir,
							 double x_rot, double z_rot, 
							 ...
							 double spin_x)
{
  /* cone */
    ...
    for (int k=0;k<=deg_cir;k+=DEF_D)
    {
      ...
      glTexCoord2f(.25*Cos(k-spin_x)+0.634,.45*Sin(k-spin_x)+.5); glVertex3d(Cos(k),Sin(k),0);

      glTexCoord2f(.25*Cos(k-spin_x+DEF_D)+0.634,.45*Sin(k-spin_x+DEF_D)+.5); glVertex3d(Cos(k+DEF_D),Sin(k+DEF_D),0);
    }
    glEnd();
    glPopMatrix();
}
...
/*
 * draws the section that connects the top and bottom of teh hull
 * translated x,y,z
 * scaled dx, dy, dz
 * 
 */
				   ...
				   unsigned int cap, unsigned int cyl)
{
    ...
    angle = 0.0;
        while( angle < 2*PI ) {
            ...
            glNormal3f(x,y,0);
            glTexCoord2f(x,0); glVertex3f(x, y , 0.0);
...
/*
 *  connects the bridge to the main hull of the ship
 *     at (x,y,z)
 *     radius (r)
 */
...
/*
 *  using this to draw my astroids
 *     at (x,y,z)
 *     radius (r)
 */
